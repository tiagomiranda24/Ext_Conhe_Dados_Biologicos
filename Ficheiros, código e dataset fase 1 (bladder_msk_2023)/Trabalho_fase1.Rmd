...
#### Análise estatística univariada e Caracterização dos tipos de
variaveis \### 1 Variáveis Numéricas

1.1. Idade em que a Sequenciação foi Reportada (Anos) 1.2. Sobrevivência
Global (Meses desde o Tx com Erdafitinib) 1.3. Sobrevivência Sem
Progressão (Meses desde o Tx com Erdafitinib) 1.4. Sobrevivência Global
em Meses desde o Sequenciamento
...
2.1. Detalhes do Tipo de Câncro 2.2. Estado da Doença 2.3. Tratamento
com Erdafitinibe 2.4. Status de Sobrevivência Global desde o Tratamento
com Erdafitinibe 2.5. Status de Progressão Livre desde o Tratamento com
Erdafitinibe 2.6. Raça 2.7. Classe da Amostra 2.8. Sexo 2.9. Status de
Fumador 2.10. Status Somático 2.11. Tipo de MSI (Instabilidade de
Microssatélites) 2.12. Código Oncotree
...
3.1. *Melhor Resposta ao Erdafitinib* 3.2. *Fração do Genoma Alterado*
3.3. *Contagem de Mutações* 3.4. *TMB* 3.5. *Pureza do Tumor*
...
As variáveis a serem consideradas incluem o Tempo de sobrevivência
global (Meses desde o inicio de tratamento com
Erdafitinib)(bladder_clin$ERDAFITINIB_TX_OS_MONTHS) e o sexo (bladder_clin$SEX).
...
As variáveis a serem consideradas incluem a Melhor resposta ao
Erdafitinib
(bladder_clin$BEST_RESPONSE) e o tratamento (bladder_clin$ERDAFITINIB_TREATED).
...
# Carregar um novo dataset (blca_msk_tcga_2020)
```{r}
# Obter dados clínicos
blca_clin <- clinicalData(api = cbio, studyId = "blca_mskcc_solit_2012")
blca_clin
dim(blca_clin)  # Deve retornar 97 linhas e 11 colunas

# Carrega os dados do cBioPortal
blca = cBioDataPack("blca_mskcc_solit_2012", ask = FALSE)
blca

# Verificar qual é o type das colunas
str(blca_clin) # Nota-se que são todas do tipo "chr" (character)
names(blca_clin)
...
na_columns <- sapply(blca_clin, function(x) any(is.na(x)))

for (col_name in names(blca_clin)) {
  if (na_columns[col_name]) {
    if (is.numeric(blca_clin[[col_name]])) {
      median_val <- median(blca_clin[[col_name]], na.rm = TRUE)
      blca_clin[[col_name]][is.na(blca_clin[[col_name]])] <- median_val
    } else {
      blca_clin[[col_name]][is.na(blca_clin[[col_name]])] <- "Unknown"
    }
  }
}
...
Normalização dos dados (blca_msk_tcga_2020)
```{r}
# Normalização dos dados
# Definida uma função "function(x) para Min-Max Scaling
min_max_scaling <- function(x) {
...
# Min-Max Scaling é aplicada exclusivamente a colunas numéricas
for (col in names(blca_clin)) {
  if (is.numeric(blca_clin[[col]])) {
    blca_clin[[col]] <- min_max_scaling(blca_clin[[col]])
  }
}
...
blca_methylation_hm27 <- as.data.frame(assays(blca)$methylation_hm27)
blca_mrna_agilent_microarray <- as.data.frame(assays(blca)$mrna_agilent_microarray)
blca_mrna_agilent_microarray_zscores_ref_all_samples <- as.data.frame(assays(blca)$mrna_agilent_microarray_zscores_ref_all_samples)
blca_mrna_agilent_microarray_zscores_ref_diploid_samples <- as.data.frame(assays(blca)$mrna_agilent_microarray_zscores_ref_diploid_samples)
blca_mutations <- as.data.frame(assays(blca)$mutations)
```
