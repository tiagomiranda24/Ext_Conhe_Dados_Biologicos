...
# Redução de dimensionalidade com PCA - torna o processo mais eficiente
...
-Convertemos a variável categórica DFS_STATUS em fator apenas para os dados de treino.
...
```
-Definimos as funções para calcular o RMSE (Root Mean Squared Error) e o MAD (Mean Absolute Deviation).
-Preparamos os dados dividindo-os em conjuntos de treino e teste.
-Convertemos a variável de resposta DFS_STATUS em fator para ambos os conjuntos de dados.
-Extraimos o nome das variáveis preditoras.
-Normalizamos os dados de treino e teste.
-Realizamos a modelagem e avaliação com o algoritmo Naive Bayes.
-Extraimos as variáveis preditoras do conjunto de dados de teste.
-Garante que os níveis das variáveis categóricas no conjunto de dados de teste correspondem aos níveis no conjunto de dados de treino.
...
-Para o Naive Bayes, o código realiza a previsão usando o modelo model_nb e calcula a tabela de confusão e a taxa de erro de classificação (PECC). No entanto obtemos um erro que nos impossibilita de avançar.
-Para as Árvores de Decisão, são criados dois modelos: um sem poda (blad_ctree) e outro com poda (tree2). A previsão é feita para ambos os modelos e, em seguida, são calculadas as tabelas de confusão e as taxas de erro de classificação (PECC).
-Para a Árvore de Regressão, o modelo é criado utilizando a variável AGE como variável de resposta. A previsão é feita e são calculados o RMSE e o MAD.
-Os resultados são apresentados em um dataframe (results) e são exibidos na tela, junto com os valores de RMSE e MAD para a Árvore de Regressão.
-Realizamos o plot das árvores de decisão e regressão.
```{r}
