...
## Visualização da Normalização dos dados
```{r}
...
# Pairplot PÓS-NOMARLIZAÇÃO (repetição do código anterior do pairplot1, mas definido como "pairplot2" para fazer comparações)
numeric_data <- bladder_clin[, sapply(bladder_clin, is.numeric)]
...
set.seed(12345)
ind <- sample(2, nrow(bladder_clin), replace = TRUE, prob = c(0.7, 0.3))
trainData <- bladder_clin[ind == 1, ]
testData <- bladder_clin[ind == 2, ]

# Modelagem e avaliação com k-NN
tryCatch({
  # Verifica se há valores NA nos dados
  if(anyNA(trainData) || anyNA(testData)) {
    stop("Dados contêm valores NA. Por favor, trate os valores ausentes antes de prosseguir.")
  }
  
  # Modelagem com k-NN
  knn_pred <- knn(trainData[, 1:4], testData[, 1:4], trainData$DFS_STATUS)
  t_knn <- table(knn_pred, testData$DFS_STATUS)
  pecc_knn <- pecc(knn_pred, testData$DFS_STATUS)
  sensib_knn <- t_knn[2, 2] / sum(testData$DFS_STATUS == "DiseaseFree")  # Sensibilidade
  especif_knn <- t_knn[1, 1] / sum(testData$DFS_STATUS == "Disease")  # Especificidade
  
  # Apresentação dos resultados
  results <- data.frame(
    Model = "k-NN",
    PECC = pecc_knn,
    Sensibilidade = sensib_knn,
    Especificidade = especif_knn
  )
  print(results)
  
}, error = function(e) {
  cat("Erro durante a modelagem e avaliação com k-NN:", conditionMessage(e), "\n")
})
```
- Modelagem e Avaliação com Naive Bayes: Um modelo de Naive Bayes é treinado com os dados de treino e usado para fazer previsões nos dados de teste. São calculadas métricas de desempenho, como PECC.

- Modelagem e Avaliação com Árvores de Decisão: Uma árvore de decisão é construída com os dados de treino e usada para fazer previsões nos dados de teste. São calculadas métricas de desempenho, como PECC.

- Apresentação dos Resultados: Os resultados das avaliações são organizados em um dataframe e impressos na tela.
```{r}
# Modelagem e avaliação com Naive Bayes
model_nb <- naiveBayes(DFS_STATUS ~ ., data = trainData)  # Modelagem do Naive Bayes
nb_pred <- predict(model_nb, newdata = testData)  # Previsões do modelo
t_nb <- table(nb_pred, testData$DFS_STATUS)  # Tabela de confusão
pecc_nb <- pecc(nb_pred, testData$DFS_STATUS)  # Cálculo do PECC

# Modelagem e avaliação com Árvores de Decisão
blad_ctree <- ctree(DFS_STATUS ~ ., data = trainData)  # Modelagem da árvore de decisão
testPred_ctree <- predict(blad_ctree, newdata = testData)  # Previsões da árvore
t_ctree <- table(testPred_ctree, testData$DFS_STATUS)  # Tabela de confusão
pecc_ctree <- pecc(testPred_ctree, testData$DFS_STATUS)  # Cálculo do PECC

# Apresentação dos resultados
results <- data.frame(
  Model = c("k-NN", "Naive Bayes", "Árvores de Decisão"),
  PECC = c(pecc_knn, pecc_nb, pecc_ctree),
  Sensibilidade = c(sensib_knn, NA, NA),  # Sensibilidade não calculada para Naive Bayes
  Especificidade = c(especif_knn, NA, NA)  # Especificidade não calculada para Naive Bayes
)
print(results)
```
- Árvore de Decisão com Formulação Explícita: Uma árvore de decisão é construída explicitamente usando a fórmula especificada e os dados de treino. A árvore é plotada e suas previsões são avaliadas.

- Árvore de Decisão com o Pacote 'tree': Uma árvore de decisão é construída usando o pacote 'tree' e os dados especificados. A árvore é plotada e suas previsões são avaliadas.

- Podar a Árvore: A árvore construída anteriormente é podada para melhorar seu desempenho e reduzir o overfitting.

- Árvore de Regressão com o Pacote 'rpart': Uma árvore de regressão é construída usando o pacote 'rpart' e os dados especificados. A árvore é plotada e suas previsões são avaliadas usando RMSE e MAD.
```{r}
# Árvore de Decisão com formulação explícita
formula <- DFS_STATUS ~ TUMOR_STAGE
blad_ctree <- ctree(formula, data = trainData)
plot(blad_ctree)
testPred2 <- predict(blad_ctree, newdata = testData)
t_ctree2 <- table(testPred2, testData$DFS_STATUS)        
pecc(testData$DFS_STATUS, testPred2)

# Árvore de Decisão com o pacote 'tree'
tree1 <- tree(DFS_STATUS ~ . - TUMOR_STAGE, data = bladder_clin) 
plot(tree1)
text(tree1)

plot(bladder_clin$DFS_STATUS, bladder_clin$TUMOR_STAGE, pch = 19, col = as.numeric(bladder_clin$DFS_STATUS))
partition.tree(tree1, label = "DFS_STATUS", add = TRUE)
legend(2, 4.5, legend = unique(bladder_clin$DFS_STATUS), col = unique(as.numeric(bladder_clin$DFS_STATUS)), pch = 19)
...
# Árvore de Regressão com o pacote 'rpart'
arvreg <- rpart(TUMOR_STAGE ~ syct + mmin + mmax + chmax + chmin, data = TUMOR_STAGE_Tr)
plot(arvreg)
text(arvreg)
val_prev <- predict(arvreg, newdata = TUMOR_STAGE_Ts)
val_prev
rmse(val_prev, TUMOR_STAGE_Ts$TUMOR_STAGE)
mad(val_prev, TUMOR_STAGE_Ts$TUMOR_STAGE)
```
