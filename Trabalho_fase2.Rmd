...
Há várias considerações importantes a serem feitas para determinar se o PCA é útil ou não neste caso. Os dados incluem uma variedade de variáveis demográficas e clínicas comuns nas investigações médicas e biológicas. 
Aqui, o PCA pode ser útil para visualizar e entender como essas variáveis separadas podem estar relacionadas ou conectadas. Este é mais eficaz quando se trata de variáveis contínuas, porque a redução de dimensionalidade pode ajudar a mostrar padrões ocultos.
O foco principal parece estar na variabilidade genética e na resposta ao tratamento. Assim, torna-se mais relevante análises de expressão genetica e diferenças moleculares relacionadas com os tratamentos ou subtipos de cancro. 
Tentamos incluir PCAs de dados de expressão genetica para identificar diferenças entre tumores primários e secundários ou para identificar subgrupos de pacientes usando perfis de expressão.

### Isto que se segue não é para ficar no trabalho final, só um tutorial/orientações para os passos seguintes nesta parte:

Após um PCA de sucesso é preciso:

  Validar a Variância Explicada:
  # Realizar PCA e verificar a variância explicada
pca_result <- prcomp(assay(vsd), scale. = TRUE)
summary(pca_result)  # Mostra a importância dos componentes, incluindo a variância explicada
  # Extrair e plotar a variância explicada para visualização
var_exp <- pca_result$sdev^2 / sum(pca_result$sdev^2)  # proporção da variância explicada
plot(var_exp, xlab = "Componentes Principais", ylab = "Variância Explicada", type = 'b',
     main = "Gráfico de Scree dos Componentes Principais")
     
  Integração com Outras Análises:
  # Assumindo que o PCA já foi realizado e os resultados estão em `pca_result`
  # Usar os dois primeiros componentes principais para um plot de cluster
plot(pca_result$x[,1:2], col = colData(vsd)$some_grouping_factor, pch = 19,
     xlab = "PC1", ylab = "PC2", main = "PCA Plot com Coloração por Grupo")
legend("topright", legend = levels(factor(colData(vsd)$some_grouping_factor)), col = 1:nlevels(factor(colData(vsd)$some_grouping_factor)), pch = 19)
  # Após identificar grupos interessantes, realizar análise de expressão diferencial
  # entre grupos selecionados ou análise de enriquecimento funcional
  
  Visualização Melhorada: Plotar as Cargas dos Componentes Principais (Loadings):
  # Plotar as cargas dos componentes principais
loadings <- pca_result$rotation  # Obtém as cargas
biplot(pca_result, cex = 0.5)  # Biplot que mostra tanto scores quanto cargas
  # Alternativamente, plotar apenas as maiores cargas para os primeiros dois PCs
top_loadings <- head(order(abs(loadings[,1]), decreasing = TRUE), 10)
barplot(loadings[top_loadings, 1], names.arg = rownames(loadings)[top_loadings],
        main = "Top 10 Cargas para PC1", las = 2)
...
```{r}
######### Acho que ficou resolvido o erro falta analisar e escrever sobre eles, ou sobre os que queremos que fiquem no trabalho e aprofundar/fazer esses! Vejam a descrição acima para tirarem ideias.

########## Principal component plot of the samples ##########

# Calcula o PCA com base nos dados de expressão ou outro conjunto relevante
pca_result <- prcomp(assay(vsd), scale. = TRUE)
exp_var <- summary(pca_result)$importance["Proportion of Variance",]

# Plota a "variância explicada" para visualizar a contribuição de cada componente
plot(exp_var, type = 'b', main = "Variância Explicada por Cada Componente Principal", xlab = "Componente Principal", ylab = "Variância Explicada")

# Identifica agrupamentos baseados nos dois primeiros componentes principais
clustering_results <- kmeans(pca_result$x[, 1:2], centers = 3)

# Visualiza os agrupamentos
plot(pca_result$x[, 1:2], col = clustering_results$cluster, pch = 19, xlab = "PC1", ylab = "PC2")
legend("topright", legend = paste("Cluster", 1:3), fill = 1:3)

# Obtem as cargas dos componentes principais
loadings <- pca_result$rotation

# Visualiza as cargas para os dois primeiros componentes principais
plot(loadings[, 1], loadings[, 2], type = 'n', xlab = "PC1 Loadings", ylab = "PC2 Loadings", main = "PCA Loadings Plot")
text(loadings[, 1], loadings[, 2], labels = rownames(loadings), cex = 0.7)

```

```{r}
# Eram tantos PCAs que decidi criar uma função para a maioria deles, no fim está o treatment_type que precisou de um pequeno pré-processamento dos dados, se for preciso consigo reverter facilmente para plotar grafico a grafico.

# Para gerar mais PCAs one-by-one:
#plotPCA(vsd, intgroup = c("escolher a coluna certa, as colunas que estão sub-divididas precisam do tratamento certo para serem visualizadas no PCA"))

# Com base em alguns PCAs anteriores estes foi os que gerei:

# Função para realizar e plotar PCAs
run_and_plot_PCA <- function(vsd, variable, title) {
  # Verificar se a variável existe em colData(vsd)
  if (variable %in% colnames(colData(vsd))) {
    # Executar PCA usando a variável correta para agrupamento
    pca_result <- plotPCA(vsd, intgroup = variable, ntop = 500, returnData = TRUE) #usei as 500 top features que são o standard mas podemos mudar mediante a variavel
    # Checar se o pca_result é um dataframe antes de tentar plotar
    if (is.data.frame(pca_result)) {
      # Usar ggplot para criar o gráfico PCA
      ggplot(pca_result, aes(x = PC1, y = PC2, color = !!rlang::sym(variable))) +
        geom_point() +
        ggtitle(title) +
        theme_minimal()
    } else {
      print("O resultado de plotPCA não é um dataframe.")
    }
  } else {
    print(paste("A coluna", variable, "não está disponível em colData(vsd)."))
  }
}

# Executar PCA para diferentes variáveis demográficas e clínicas
run_and_plot_PCA(vsd, "gender", "PCA para a variável Gênero") #nao sei se retire
run_and_plot_PCA(vsd, "race", "PCA para a variável Raça") #nao sei se retire
run_and_plot_PCA(vsd, "ethnicity", "PCA para a variável Etnia") #nao sei se retire
run_and_plot_PCA(vsd, "vital_status", "PCA para a variável Estado Vital")
run_and_plot_PCA(vsd, "age_at_index", "PCA para a variável Idade no diagnóstico")
run_and_plot_PCA(vsd, "ajcc_pathologic_stage", "PCA para a variável Estágio da Doença")

# Código para tratamento tipo de dados
colData(dds)$simple_treatment_type <- sapply(colData(dds)$treatments, function(x) x$treatment_type[1])
colData(vsd) <- colData(dds)
run_and_plot_PCA(vsd, "simple_treatment_type", "PCA por Tipo de Tratamento")
```
