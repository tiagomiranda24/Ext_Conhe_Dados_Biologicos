...
motivos, como erros de medição ou falhas na coleta de dados, e é
...
colSums(is.na(bladder_clin)) # Verifica quantos valores NA cada coluna tem
...
O tratamento dos NAs, ou valores ausentes, é uma etapa crucial no
processo de análise de dados. Os NAs podem surgir por uma variedade de
razões, como erros de medição, falhas na coleta de dados ou simplesmente
pela natureza dos dados.

Os NAs das colunas numéricas são substituídos pela mediana.

Relativamente às variáveis não numéricas substituímos por um valor
"Unknown" (Desconhecido) para em vez de dar erro nos gráficos, criar uma
variável à parte para valores que não podem ser integrados.
...
A normalização dos dados é um procedimento essencial em análises
estatísticas e machine learning. O ajuste dos valores das variáveis
ajuda a garantir que todas as variáveis contribuam igualmente para o
modelo. Essa etapa preparatória é fundamental para obter resultados mais
confiáveis e interpretações mais precisas
...
for (col in names(bladder_clin)) {
  if (is.numeric(bladder_clin[[col]])) {
    bladder_clin[[col]] <- min_max_scaling(bladder_clin[[col]])
  }
}
```
...
A função as.factor foi utilizada para converter variáveis categóricas
num formato adequado para análise estatística sendo essencial para
garantir que essas variáveis sejam tratadas corretamente pelos
algoritmos e modelos. A função as.factor atribui um código numérico
único para cada categoria da variável, facilitando a interpretação e
manipulação dos dados. Ao definir as variáveis com as.factor, garantimos
uma representação precisa das relações entre as categorias, permitindo
uma análise estatística mais robusta e interpretações mais claras dos
resultados.
...
Neste ponto é realizada uma análise sumária de algumas variaveis de cada categoria de forma a comprender melhor o publico alvo presente no estudo.
...
1.  Remover valores ausentes e calcular estatísticas descritivas:
    -   Primeiro, os valores ausentes (NaNs) da coluna com variáveis
        numéricas são removidos do conjunto de dados.
    -   Em seguida, são calculadas algumas estatísticas descritivas:
        -   Média dos valores da variável: Calcula-se a média dos
            valores da variável presentes no conjunto de dados limpo.
        -   Mediana dos valores da variável: Determina-se a mediana dos
            valores da variável.
        -   Desvio padrão dos valores da variável: Calcula-se o desvio
            padrão dos valores da variável.
        -   Intervalo interquartílico (IQR): O IQR é a diferença entre o
            terceiro quartil (Q3) e o primeiro quartil (Q1) dos valores
            da variável.
2.  Barplot e boxplot:
    -   Um gráfico de barras (barplot) é criado para visualizar a
        distribuição dos valores da variável.
    -   Um gráfico de caixa (boxplot) é gerado para mostrar a variação e
        os possíveis valores atípicos dos valores da variável.
...
A análise da sobrevivência global dos pacientes desde o início do
tratamento com Erdafitinib revelou informações importantes. Após a
remoção dos NAs, calculamos as estatísticas descritivas, incluindo o
desvio padrão e o intervalo interquartílico. Além disso, representamos
graficamente os dados usando um barplot e um boxplot.
...
A análise da sobrevivência sem progressão dos pacientes desde o início
do tratamento com Erdafitinib revela informações importantes sobre a
eficácia do medicamento. Após a remoção dos NAs calculamos estatísticas
descritivas, como desvio padrão e intervalo interquartílico. Além disso,
representamos graficamente os dados usando um barplot e um boxplot,
permitindo-nos visualizar a distribuição dos períodos de sobrevivência
sem progressão dos pacientes. Essa análise ajuda-nos a compreender
melhor a eficácia do tratamento com Erdafitinib em retardar a progressão
da doença.
...
1.  Gráfico de barras para visualizar as frequências absolutas:
    -   Um gráfico de barras é criado para representar a frequência
        absoluta de cada categoria presente nos dados.
2.  Cálculo das frequências absolutas e relativas:
    -   Primeiro, as frequências absolutas das diferentes categorias são
        calculadas.
    -   Em seguida, as frequências relativas são obtidas dividindo as
        frequências absolutas pelo total de observações.
3.  Criação de um data frame com as frequências absolutas e relativas:
    -   Um data frame chamado freq_data é criado, contendo três colunas:
        ...
        -   Frequencia_Absoluta: As frequências absolutas
            correspondentes.
        -   Frequencia_Relativa: As frequências relativas
            correspondentes.
4.  Gráfico de barras para as frequências absolutas:
    -   Um novo gráfico de barras é gerado para representar as
        frequências absolutas das categorias.
5.  Gráfico de barras para as frequências relativas:
    -   Outro gráfico de barras é criado para mostrar a distribuição
        relativa das categorias.
    -   O título do gráfico é "Distribuição Relativa por Categoria".
    -   O eixo x é rotulado como "Categoria" e o eixo y é rotulado como
        "Frequência Relativa".
6.  Gráfico de pizza:
    -   Por fim, um gráfico de pizza é gerado para ilustrar a
        distribuição relativa das categorias.
    -   Os rótulos das fatias da pizza incluem o nome da categoria e a
        porcentagem correspondente arredondada.
...
1.  Resumo estatístico: Utiliza a função `summary()` para calcular
    estatísticas descritivas, como mínimo, 1º quartil, mediana, média,
    3º quartil e máximo da variável dependente. Os valores ausentes são
    removidos antes do cálculo (`na.omit()`).

2.  Desvio padrão: Calcula o desvio padrão da variável dependente
    usando a função `sd()`. Novamente, os valores ausentes são removidos
    antes do cálculo.

3.  Intervalo interquartílico (IQR): Calcula o intervalo
    interquartílico da variável dependente usando a função `IQR()`. Os
    valores ausentes são removidos antes do cálculo.

4.  Visualização de frequência: Plota um histograma de frequência
    dos valores da variável dependente usando a função
    `plot(table(...))`.

5.  Gráfico de caixa (boxplot): Cria um gráfico de caixa para
    visualizar a distribuição dos valores da variável dependente. Os
    valores ausentes são removidos antes de criar o gráfico.


#### 3.1. Melhor Resposta ao Erdafitinib
```{r}
# Remover valores ausentes e calcular o valor minimo, 1º quartil, mediana, média, 3º quartil e valor máximo da variavel.
summary(na.omit(bladder_clin$BEST_RESPONSE))

# Remover valores ausentes e calcular o desvio padrão da variavel.
sd(na.omit(bladder_clin$BEST_RESPONSE))

# Remover valores ausentes e calcular o Intervalo interquartilico.
IQR(na.omit(bladder_clin$BEST_RESPONSE))
...
```{r}
# Remover valores ausentes e calcular o valor minimo, 1º quartil, mediana, média, 3º quartil e valor máximo da variavel.
summary(na.omit(bladder_clin$FRACTION_GENOME_ALTERED))

# Remover valores ausentes e calcular o desvio padrão da variavel.
sd(na.omit(bladder_clin$FRACTION_GENOME_ALTERED))

# Remover valores ausentes e calcular o Intervalo interquartilico.
IQR(na.omit(bladder_clin$FRACTION_GENOME_ALTERED))

# Calcular a variância
var(na.omit(bladder_clin$FRACTION_GENOME_ALTERED))

filtered_data <- bladder_clin[complete.cases(bladder_clin$FRACTION_GENOME_ALTERED), ] # Filtrar os dados nao numericos

boxplot(filtered_data$FRACTION_GENOME_ALTERED, horizontal = TRUE) # Gráfico boxplot
```
#### 3.3. Contagem de Mutações
```{r}
# Remover valores ausentes e calcular o valor minimo, 1º quartil, mediana, média, 3º quartil e valor máximo da variavel.
summary(na.omit(bladder_clin$MUTATION_COUNT))

# Remover valores ausentes e calcular o desvio padrão da variavel.
sd(na.omit(bladder_clin$MUTATION_COUNT))

# Remover valores ausentes e calcular o Intervalo interquartilico.
IQR(na.omit(bladder_clin$MUTATION_COUNT))

# Calcular a variância
var(na.omit(bladder_clin$MUTATION_COUNT))

# Gráficos
barplot(table(bladder_clin$MUTATION_COUNT))
boxplot(bladder_clin$MUTATION_COUNT, horizontal = TRUE)
```
# Relação Tempo de sobrevivência global e o sexo

Nesta fase, iremos realizar uma análise estatística univariada, bem como
análises de expressão diferencial e de enriquecimento.
...
```
### ANOVA Para avaliar se o Tempo de Sobrevivência Global desde o início
do tratamento para cada Sexo, foi considerada a avaliação da variável
transformada. Desenhamos o gráfico boxplot que permita efetuar a
comparação da distribuição do logaritmo do tempo de Sobrevivência Global
desde o início do tratamento.

Testar a hipótese nula 𝐻0: 𝜇masculino = 𝜇feminino, onde μ indica o valor
médio para cada sexo. É sugerida a utilização de uma ANOVA para a
variável transformada (log(x)).

Verificar os pressupostos para poder efetuar a dita análise.
Consideramos α=1% (Utilizamos também o teste de Shapiro (normalidade) e
o teste de Fligner(homogeneidade)).
...
Pretendemos avaliar se o Tempo médio de Sobrevivência Global desde o
início do tratamento varia para cada Sexo. Para tal, sugere-se a
realização de uma ANOVA que permita avaliar os efeitos principais e a
possível interação entre os grupos. Que pode concluir (use α = 5%).
```{r}
...
any(is.na(bladder_clin$ERDAFITINIB_TX_OS_MONTHS) | is.infinite(bladder_clin$ERDAFITINIB_TX_OS_MONTHS))
any(is.na(bladder_clin$SEX) | is.infinite(bladder_clin$SEX))
```
...
### T-test (teste das médias) Efetuamos o teste estatístico mais
apropriado, neste caso o t-test os valores médios de Melhor Resposta ao
Erdafitinib para os individuos que estão a fazer o tratamento e para os
que não estão a fazer.

Hipóteses: H0: Não existem diferenças no valor médio da melhor resposta
ao tratamento para cada grupo H1: Existem diferenças no valor médio da
melhor resposta ao tratamento para cada grupo

Teste t para comparar os valores médios da melhor resposta ao tratamento
para cada grupo
...
```
#Carregar um novo dataset

Tal como referimos no e-mail enviado, inicialmente selecionamos um dataset para o trabalho, data set que foi utilizado para todo o trabalho até este momento. 
No entanto à data de envio do e-mail, apercebemo-nos que o dataset escolhido não continha valores para o RNA-Seq.
Estes valores são cruciais para conduzir a análise diferencial e a análise de enriquecimento, sendo que assim ficamos impossibilitados de prosseguir com o nosso plano original. 
Decidimos, então, realizar a análise diferencial utilizando os dados de um outro dataset que abordava o mesmo tema.
```{r}
...
BiocManager::install("GDCRutils")
```


```{r}
# Carregar o pacote GDCRutils
library(GDCRutils)
...
# Verificar se o arquivo luad_data_load.rda já existe
if (!file.exists("luad_data_load.rda")) {  
  # Se o arquivo não existe, baixar os dados e preparar
  GDCdownload(luad)
  ...
  # Se o arquivo já existe, carregar os dados diretamente
  load("luad_data_load.rda")
}

# Acessar os dados do assay 'unstranded' e convertê-los em um data frame
seqdata <- as.data.frame(assay(luad_data, 'unstranded'))

```
## Remoção de Valores omissos, Normalização e logaritmização dos dados
```{r}
# Remover linhas com valores ausentes
seqdata <- na.omit(seqdata)

# Normalização Min-Max
normalized_data <- apply(seqdata, 2, function(x) (x - min(x)) / (max(x) - min(x)))

# Logaritmização
log_normalized_data <- log(normalized_data + 1)  # Adicionando 1 para evitar log(0)
# Agora log_normalized_data contém os dados normalizados e logaritmizados
...
str(log_normalized_data)

# Verificar as dimensões do objeto
dim(log_normalized_data)
...
## Filtragem dos dados
```{r}
# Calcular os máximos e mínimos para cada gene
maximos <- apply(log_normalized_data, 1, max)
minimos <- apply(log_normalized_data, 1, min)

# Identificar os genes cujo rácio de máximo para mínimo é maior que 2
vl <- maximos / minimos > 2

# Filtrar o conjunto de dados com base no critério de rácio de máximo para mínimo
log_normalized_datam2 <- log_normalized_data[vl,]

# Selecionar as amostras relacionadas com as condições "BCR/ABL" ou "NEG"
s <- grep("BCR/ABL|NEG", colnames(log_normalized_data))

```


```{r}
nome_primeira_coluna <- colnames(log_normalized_data)[1]
nome_primeira_coluna

# Verificar a classe do objeto
class(log_normalized_data)

# Acessar a coluna "TCGA-CU-A3KJ-01A-11R-A21D-07"
log_col <- log_normalized_data[, "TCGA-CU-A3KJ-01A-11R-A21D-07"]

# Converter a variável em um fator
log_col_factor <- factor(log_col)

# Tabela de frequência da variável 
table(log_col_factor)

# Carregar a biblioteca genefilter
library(genefilter)

# Verificar o número de níveis únicos em log_col_factor
n_levels <- length(unique(log_col_factor))
print(n_levels)

# Visualizar os nomes das estatísticas calculadas
names(tt)

# Visualizar os valores de p-valor
tt$p.value

# Ordenar os valores de p-valor e selecionar os 20 primeiros
rank <- order(tt$p.value)
p20 <- rank[1:20]

# Visualizar os valores de p-valor dos 20 genes com menor p-valor
print(tt$p.value[p20])

# Selecionar as 20 primeiras linhas com menor p-valor
L20 <- log_normalized_data[p20,]

# Ordenar as colunas com base na condição (se necessário)
order_cols <- order(L20[, "TCGA-CU-A3KJ-01A-11R-A21D-07"])
L20 <- L20[, order_cols]

# Criar o heatmap
heatmap(as.matrix(L20), Colv = NA, labCol = L20[, "TCGA-CU-A3KJ-01A-11R-A21D-07"])
