...
pie(freq_rel, main = "Distribuição Relativa por Categoria",
    ...
    cex = 0.8)
```
2.3. Tratamento com Erdafitinib
```{r}
# Gráfico de barras para visualizar as frequências absolutas
...
pie(freq_rel, main = "Distribuição Relativa por Categoria",
    ...
    cex = 0.8)
```
2.4. Status de Sobrevivência Global desde o Tratamento com Erdafitinib
```{r}
# Gráfico de barras para visualizar as frequências absolutas
...
Considerando as variáveis x e y:
Realizamos as estatísticas sumárias de x para cada y.
```{r}
# Estatísticas sumárias de x para cada nível de y
est_sumárias <- tapply(x, y, summary, na.rm = TRUE)
# ou
by(x, y, summary)

# b) Represente a boxplot de x para cada nível de y)
boxplot(x ~ y, col = 0)

# c) Será que os dois grupos apresentam a mesma variância?
# Hipóteses:
# H0: A variância é a mesma nos grupos standard e de teste
# H1: A variância não é a mesma
...
Efetuamos o teste estatístico mais apropriado, neste caso o t-test os valores médios de x face a y.
# Hipóteses:
# H0: Não existem diferenças
# H1: Existem diferenças no x dependendo do y

teste_t <- t.test(x ~ y, var.equal = TRUE)
teste_t
# Decisão de acordo com o valor-p
#RESPOSTA: VALOR DA ESTATISTICA DE TESTE: F(...)=... ; decisão de acordo com valor de p

Efectuamos  o teste estatístico mais apropriado para os valores médios de 
    x face a y (Escrever as hipóteses, temos de indicar o valor da estatística de teste e )
      #apresente a decisão para um α=5%).
    
    #H0: não existem diferenças
    #H1: existem diferenças no x dependendo do y
    teste_t=t.test(x~y, var.equal=T)
    teste_t
    #RESPOSTA; ET-> t(...)=...; decisão segundo p.value
    

Considerando agora a distribuição de x/y (execute o comando table()). --> atenção que se a variavel estiver em texto, se tiver codificado temos de declarar as a factor tal como já foi feito
    #Pensa-se que a distribuição de x é a mesma em cada categoria(y).  ---> ou seja, distribuiçao uniforme (p=/)
    
    #Determine as frequências esperadas para o x de acordo com as probabilidades teóricas.
    ...
    #bConstruimos o gráfico de barras que compare as frequências observadas com as esperadas.
    barplot(rbind (tabela,ei), beside = T, col=c(2,3))
    
    #Confirmar se x segue uma distribuição uniforme? (Escrever as hipóteses,
    #indicar o valor da estatística de teste e o valor prova e decisao considerando α=5%).
    
    #H0: a distribuiçao de x é uniforme
    #H1: a distribuiçao de x não é uniforme
    
    teste_qui= chisq.test(tabela, p=pi)
    
#Verificamos se existe associação entre o x e y.
    #Escrevemos as hipóteses, indicamos o valor da estatística de teste e apresente a decisão para um  α=5%)
    #H0: o x e o y sao independentes 
    #H1: o x e o y não sao independentes
    
    m=table(x,y)   ##-->para tratar os dados e colocá-los em tabela    -->vemos se está certo pelos graus de liberdade que depois nos dá no resultado do teste
    teste_ind=chisq.test(m)
    teste_ind
    
#Para avaliar se o tempo de x médio é o mesmo para cada grupo y, foi considerada a avaliação da variável transformada, logaritmo de x
    #(log(time)), segundo cada grupo de células. 
    # Desenhamos o gráfico boxplot que permita efetuar a comparação da distribuição do logaritmo do tempo de sobrevida segundo o tipo de células
    
    boxplot(log(x)~y, col=6)
    
    
  #Testamos a hipótese nula 𝐻0: 𝜇1 = 𝜇2 = 𝜇3 = 𝜇4, onde i indica o x médio para cada grupo de y. É sugerida a utilização de uma ANOVA para a variável transformada (log(x)). Verifique os pressupostos para poder efetuar a dita análise. Consideramos α=1%. (Utilizamos tambem o teste de shapiro e o teste de fligner)
  ...
  # depois de confirmar os pressupostos de normalidade e homogeneidade realizamos a anova
    modelo=aov(log(x)~y)
    summary(modelo)
    TukeyHSD(modelo, conf.level=0.99)      #comparações múltiplas
    plot(TukeyHSD(modelo, conf.level = 0.99))
    
# Pretendemos avaliar se x médio varia de acordo com o grupo de y e o z. Para tal sugere-se a realização de uma ANOVA que permita avaliar os efeitos principais e a possível interação entre o tipo de x e y utilizada. Que pode concluir (use α=5%)
    ...
    #RESPOSTA: os resultados dos modelos anova rwoway permitem nos concluir que existem ou não efeitos significativos no x medio nos grupos y e tambem no z como podemos ver pelo interaction plot)
